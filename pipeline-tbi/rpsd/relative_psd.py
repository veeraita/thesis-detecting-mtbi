...
def calc_relative_psd(subj, output_dir, tasks=['EC']):
    ...
    for task in tasks:
        for i in range(40, 390, 50):
            ...
            outfile = os.path.join(psd_dir, f'{subj}-{task}-{i}-relative-psd-fsaverage')
            new_stc.save(outfile)
            visualize_psd(outfile, os.path.join(fig_dir, f'{subj}-{task}-{i}-relative-psd-fsaverage.png'), subj)
